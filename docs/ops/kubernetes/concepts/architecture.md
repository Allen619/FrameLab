---
title: 架构概览 | Kubernetes 教程
description: 理解 Kubernetes 的整体架构，控制平面和工作节点的组成与职责
---
...
```mermaid
flowchart TD
    A[1. kubectl 发送请求] --> B[2. API Server 接收请求]
    B --> C[3. 写入 etcd]
    C --> D[4. Scheduler 发现未调度 Pod]
    D --> E[5. Scheduler 选择节点]
    E --> F[6. Kubelet 发现新任务]
    F --> G[7. Container Runtime 拉取镜像]
    G --> H[8. 启动容器]
    H --> I[9. Kubelet 汇报状态]
```
